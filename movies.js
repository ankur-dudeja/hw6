...
window.addEventListener('DOMContentLoaded', async function(event) {
  ...
  // ⬇️ ⬇️ ⬇️
  let response = await fetch('https://api.themoviedb.org/3/movie/now_playing?api_key=5f90397550d03624cb9db441a80c1508&language=en-US')
  ...
  for (let i=0; i<movies.length; i++) {
    ...
    let movieDoc = await db.collection('hw6').doc(`${movieId}`).get()
    console.log(movieDoc + "test")
    if(movieDoc.data()) {
  ...
  // ⬇️ ⬇️ ⬇️
    ...
    document.querySelector(`.watched-button-${movieId}`).addEventListener('click', async function(event) {
      ...
      let docRef = await db.collection('hw6').doc(`${movieId}`).set({})
  ...
  let querySnapshot = await db.collection('hw6').get()
  console.log(querySnapshot.size)

  let watchedMovies = querySnapshot.docs

  for(i=0; i<watchedMovies.length; i++) {
    let watchedMoviesID = watchedMovies[i].id
